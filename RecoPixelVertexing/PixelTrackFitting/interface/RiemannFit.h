...
namespace Rfit {
		...
		const VectorNd& rad) 
	{
		...
		for (u_int i = 0; i < n; ++i) {
			...
			if (rad(i) < 1.e-4)
				...
				cov_rad(i, i) = rad_inv2(i) * (cov_cart(i, i) * sqr(p2D(1, i)) + cov_cart(i + n, i + n) * sqr(p2D(0, i)) -
	...
	* The following computing is under MultiScattering case
	...
	\param Vr_inv the inverse of the COV matrix of othoganal distances on the unfold cylinder (after rotation)

	\return weight points' weights' vector for the line fit (ODR).
	*/


	CUDA_HOSTDEV inline VectorNd Weight_line_scatter(const Matrix2Nd& Vr_inv) {
	...
	*/
	...
	x axis for the line fit. If errors have not been computed in the circle fit with MultiScattering Case
	than an'approximation is made.
	Further information in attached documentation.
	...
	*/
	...
	CUDA_HOSTDEV inline MatrixNd X_cov(const Matrix3Nd& V, const circle_fit& circle, const MatrixNx2Np3d& J,
	const bool& error, u_int n, const u_int zbn, Matrix2Nd Scatterxy) 
	{
		MatrixNd x_cov(n, n);
	...
	CUDA_HOSTDEV inline MatrixNd Scatter_cov_z(const Matrix2xNd& p2D,
		const Vector4d& fast_fit,
		...
		const double factor) 
	{
		...
		Rfit::printIt(&scatter_cov_z, "Scatter_cov_z - scatter_cov_z: ", DEBUG);
	...
	*/
	...
	CUDA_HOSTDEV inline Matrix2Nd Scatter_cov_rtoxy(const Matrix2xNd& p2D,
		const Matrix3xNd& hits,
		const Vector4d& fast_fit,
		const circle_fit& circle,
		double B,
		const u_int nn,
		const double factor) 
	{
		...
		Rfit::printIt(&scatter_cov_rtoxy, "Scatter_cov_rtoxy - scatter_cov_rtoxy: ", DEBUG);
	...
	*/
	...
	CUDA_HOSTDEV inline Vector2d min_eigen2D_fast(const Matrix2d& A) {
	...
	*/
	...
	CUDA_HOSTDEV inline circle_fit Circle_fit_Scatter(const Matrix2xNd& hits2D,
		...
		const bool error = true) 
	{
		...
		if (error) {
			...
			Matrix3Nd Vxyz = MatrixXd::Zero(3 * n, 3 * n);
			{
				...
				//C[2][2] = 2. * (Vcs_[0][0] * Vcs_[0][0] + Vcs_[0][0] * Vcs_[0][1] + Vcs_[1][1] * Vcs_[1][0] + Vcs_[1][1] * Vcs_[1][1]) + 4. * (Vcs_[0][0] * t00 + Vcs_[0][1] * t01 + Vcs_[1][0] * t10 + Vcs_[1][1] * t11);
	...
	*/
	...
	CUDA_HOSTDEV inline line_fit Line_fit_Scatter(const Matrix3xNd& hits,
		...
		const bool error = true) 
	{
		...
		printIt(&V, "Line_fit - Cov Matrix of s and z: ", DEBUG);
		printIt(&G, "Line_fit - G: ", DEBUG);
		...
		if (error) {
			...
			printIt(&A_cov, "line_fit - A_cov", DEBUG);
			...
			E11(0, 0) = 1.;
			Matrix<double, 1, 1> varc = v.transpose() * c0 * v + r0.transpose() * cn * r0 + (cn.cwiseProduct(c0)).sum() * E11;
			...
			J3(1, 1) = - c * (sqr(v(1)) - (sqr(v(0)) + sqr(v(1)))) / (sqr(v(1)) * sqrt(sqr(v(0)) + sqr(v(1))));
			J3(1, 2) = -sqrt(sqr(v(0)) + sqr(v(1))) * 1. / v(1);
			*/
	...
	*/
	...
	inline helix_fit Helix_fit(const Matrix3xNd& hits, 
		const Matrix3Nd& hits_cov, 
		const double B,
		const bool& error = true, 
		const bool& scattering = true) 
	{
		...
		if (scattering)
		{
			circle = Circle_fit_Scatter(hits.block(0, 0, 2, n), hits_cov.block(0, 0, 2 * n, 2 * n), fast_fit, rad, B, factor, error);
			line = Line_fit_Scatter(hits, hits_cov, circle, fast_fit, B, factor, error);
		}
		else
		{
			circle = Circle_fit(hits.block(0, 0, 2, n), hits_cov.block(0, 0, 2 * n, 2 * n), fast_fit, rad, false);
			line = Line_fit(hits, hits_cov, circle, fast_fit, error);
		}
